...
def get_media_t(linhas, temp):
    ...
    #segundo ponto: o loop só funciona quando já temos dados na tabela. temos que colocar em um if
    if len(linhas)==0:
        media_t = temp
    else:
        soma_t = temp #primeiro ponto; inicializar a soma com o dia de hoje para compensar o fato de ainda nao estar na tabela
        for linha in linhas:
            temp_da_linha = linha[1]
            soma_t += float(temp_da_linha)
        media_t = soma_t / len(linhas)  #verificamos que esta média não está certa
        # CORRIGIR!!!
    print("...feito!")
    return media_t
